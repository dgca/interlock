...
- Entry, exit, agent, script, condition, Workflow, and inline Workflow Batch nodes.
...
Workflow Batches support at most 200 items, 50 concurrent item runs, and 10 nested workflow levels shared with Workflow nodes. Each Batch owns an inline workflow and returns results in input order. The editor opens each body in a focused canvas; simultaneous expanded nested canvases and arbitrary parallel graph joins are not supported. Legacy referenced-workflow batches remain supported without migration.
