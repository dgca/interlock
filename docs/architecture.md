...
Node settings provide an Input bindings field editor with Previous step output as the default source, a same-scope node picker for Node output, and Advanced JSON. Canvas source indicators select and focus referenced nodes without adding execution edges or changing the draft. Node and Batch bounds include space for each distinct source indicator.
