...
A Switch compares the same kind of path against an ordered case list using structural JSON equality. The first match selects its named port; no match selects the required default port. A missing path fails the step. Switch passes its resolved input through unchanged, including input bindings, and validates its output contract on every route. Case and default port names must be nonblank and unique at publication, with exactly one outgoing edge each. Empty case lists are allowed. Port names are local to the source node: names such as `item` and `timeout` do not trigger Batch or Agent behavior on Switch. Executions persist the selected `port`. See [Switch routing](switch.md).
