...
One entry begins a run. Each node receives the previous node's output as its whole input. Each ordinary node has one default outgoing route. Conditions have one true route and one false route. Switch nodes have one route per named case and an optional named fallback route. Agents with `unclaimedTimeoutMs` have default and timeout routes. Exit nodes have no outgoing route. Workflow-level back-edges can target upstream nodes other than Entry. The same run re-executes those nodes, with each visit counted against `maxSteps`. Batch item paths must be acyclic.
...
A Switch compares the same kind of path against an ordered case list using structural JSON equality. The first match selects its named port; no match fails the run unless an optional `default` port specifies a fallback route. A missing path fails the step. Switch passes its resolved input through unchanged, including input bindings, and validates its output contract on every route. Case and configured fallback port names must be nonblank and unique at publication, with exactly one outgoing edge each. Omitting `default` requires no fallback edge. Empty case lists are allowed and always take the unmatched action after resolving the path. Port names are local to the source node: names such as `item` and `timeout` do not trigger Batch or Agent behavior on Switch. Executions persist the selected `port`. See [Switch routing](switch.md).

A Workflow node invokes an existing published workflow version once. Optional `mode` is `wait` or `detached`; omission retains wait behavior. Detached dispatch validates the resolved input against the referenced workflow before atomically creating a child and completing the launching execution with `{runId, workflowId, version}`. Empty output contracts use the intrinsic run-reference contract; conflicting overrides block publication. A nested storage savepoint rolls back child creation if dispatch completion fails. See [Workflow execution](workflow-execution.md). It can reference its own workflow ID, but the pinned version must already be published. A new version can therefore invoke an older version of itself, not its own unpublished definition. A Batch selects an array from `itemsPath` and repeats a visible item path for each value. A blank path selects the complete input. Each value becomes the complete input of an isolated item run. The Batch collects item results in input order, even when items finish out of order. Empty arrays produce an empty result.
...
Item paths can contain Agent, Script, Fetch, Wait, Condition, Switch, Workflow, and nested Batch nodes. Each Batch has exactly one `item` route into its direct members. Ordinary routes must remain within the same group. A Batch's `complete` route continues in its enclosing scope; a nested Batch can connect its Out handle to the enclosing Batch's End. Conditions require both branches, each leading to End. Every Switch case and any configured fallback branch must also reach End. An unmatched value without a fallback fails the item and follows the Batch failure policy. Membership is explicit graph data, independent of node positions or overlap. Validation rejects missing owners, circular membership, unreachable members, cross-group edges, Entry or Exit members, and cycles within item paths. Ordinary workflow loops outside item paths remain bounded by the workflow step limit.
...
The editor derives optional input hints from draft edges and declared contracts. Explicit node input contracts take precedence. Bindings yield a projected object shape; pass-through nodes can carry an inferred shape forward. Conflicting incoming shapes and unknown source contracts yield no single hint. These hints drive editor suggestions only. They are not stored, do not affect publication, and do not change runtime contract checks unless the author explicitly copies a hint into a node input contract.

Work and run summaries derive `workflowId`, optional `parentRunId`, `rootRunId`, and `rootWorkflowId` from persisted run ancestry. Root runs identify themselves with their root IDs and omit `parentRunId`. Summary queries cache shared ancestors within each request and read ancestor identity without loading execution history. No root fields need to be backfilled into stored runs. Detached children add `parentMode: "detached"` and `parentExecutionId`; ancestry and `rootInput` keep their original meaning. Cancellation stops at detached relationships. Detached children can be retried directly after the parent ends, and later parent retries do not repeat completed dispatches. Fresh-context instructions stop their ancestor walk at the nearest detached run. Global Runs shows detached runs alongside roots, while `rootOnly` query semantics remain unchanged. Existing JSON records need no schema migration.
...
The Add node dialog starts with Agent, the first node type, and adds a node only on confirmation. Each node type has a brief description in creation and settings. Switch settings group cases into cards with typed match values and branch names. Text does not require JSON quotes; JSON editing remains available for lists, objects, and direct value entry. Opening settings preserves stored value types. New Switch cases use the selected field's known type, or empty text when the type is unknown. **When no case matches** offers **Fail the run** or **Follow a fallback branch**; new Switch nodes default to failing, while existing configured fallbacks are preserved. The fallback name field is shown only when enabled. Disabling fallback lists its affected connection before Apply changes. Branch names remain stored as `port` values; the editor lists connections that a pending rename or removal will discard. New-choice defaults follow option order. Workflow source, script language, Fetch value sources, and boolean inputs use segmented controls; Context, Workflow execution, and Batch failure behavior use radio groups. Detached Workflow settings show the fixed Started run output contract and warn about Batch dispatch concurrency. Canvas nodes show Start and continue, and run inspection links each dispatched run to its launching execution. Detached descendants remain visible without changing the parent progress state. Workflow targets list owned children first, then library workflows, alphabetically within each group. The initial reference uses that same order. Run versions appear newest first. Saved values retain their selection regardless of option order. Script and raw-definition editors share syntax highlighting and aligned line numbers. The minimap receives its dimensions through the React Flow component's inline style so its SVG calculations match its displayed size.

Condition settings reuse the Switch typed match-value editor and explain the True and False routes. Agent tool and skill requirements use removable tags, accepted with Enter, commas, or leaving the field. Script and Fetch timeouts use an amount and unit, retain millisecond storage, and enforce the existing 100 ms to 120 second bounds. Batch settings label `itemsPath` as List to process and `concurrency` as Items at a time. These controls preserve the definition format.

Fetch fixed body fields use the shared typed value control; a whole fixed body remains in the JSON editor. Existing field values keep their JSON types. Wait timestamp paths suggest text and unknown fields while allowing free-text paths. These suggestions do not change saved definitions or execution rules.
