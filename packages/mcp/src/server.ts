...
const definitionGuide = [
  'Switch nodes use kind: "switch", path (dot-separated keys or array indices; blank selects the whole input), cases: [{port: "ticket", equals: "ticket"}], and default: "none". Cases use structural JSON equality in order; the first match selects its port and an unmatched value selects the named default port. Missing paths fail the node. Switch passes its resolved input through unchanged and supports inputBindings. Publication requires nonblank, unique case and default port names and exactly one outgoing edge per port, including default. Edge port names are scoped to the source node; names such as item and timeout have no special meaning on Switch. Empty cases are allowed and route to default after resolving path. Removing or renaming a port through JSON requires updating its edges. Switch branches inside a Batch must remain in the item scope and reach End.',
...
export function createMcpServer(client: ReturnType<typeof createMcpClient>) {
  ...
  tool(
    'create_workflow',
    'Create an editable draft from a flat definition with nodes and edges. Node kinds: entry, exit, agent, script, fetch, wait, condition, switch, workflow, batch. See the definition parameter for Switch cases and default routing, Wait timing, and Agent unclaimedTimeoutMs configuration. Batch members use batchId; its item edge starts the path, every branch returns via targetHandle end, and complete continues outside. Scripts must set language to javascript explicitly; omission means Bash. Set ownerWorkflowId to create a child of a library workflow. Children cannot own children and only their owner may reference them. Workflow nodes may use version: null in drafts until a published version is selected. This does not publish or execute it.',
    {
      name: z.string(),
  ...
  tool(
    'publish_workflow',
    'Validate the draft and publish an immutable version. Node input bindings must reference a non-Entry/Exit node in the same execution scope. Switch requires nonblank, unique case and default port names with exactly one edge per port, including its required default route. Timed Agent nodes require one default route and one timeout route; Wait nodes require one default route. Set cascade to also advance references and republish all transitive dependents from their latest published definitions, including archived workflows. Unpublished dependent definition edits or dependency cycles reject the entire operation. Existing versions and runs stay pinned. No execution is started.',
  ...
  tool(
    'get_run',
    'Inspect the published definition, status, resolved node inputs and results, immediate children, all descendants, events, and assignments without claim tokens. Claimed work is visible here even when list_work is empty. Switch executions include the selected case or default name in port and pass resolved input through unchanged. Fetch executions include resolved requests and response output. Wait executions include resumeAt as an ISO deadline. Available timed assignments include availableUntil. Timed-out Agent executions record port: timeout and assignments have status timed_out; the run may still be active on the next step. Check execution status as well as the deadline, which remains in history after completion or cancellation.',
    { id: z.string() },
