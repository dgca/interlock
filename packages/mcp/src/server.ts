...
export function createMcpServer(client: ReturnType<typeof createMcpClient>) {
  ...
  tool(
    'publish_workflow',
    'Validate the draft and publish an immutable version. Node input bindings must reference a non-Entry/Exit node in the same execution scope. Switch requires nonblank, unique case and configured default port names with exactly one edge per port. Its default fallback is optional; omitting it makes unmatched values fail the run and requires no fallback edge. Timed Agent nodes require one default route and one timeout route; Wait nodes require one default route. Set cascade to also advance references and republish all transitive dependents from their latest published definitions, including archived workflows. Unpublished dependent definition edits or dependency cycles reject the entire operation. Existing versions and runs stay pinned. No execution is started.',
