    ...
    <details>
      ...
      <Stack gap="md" mt="sm">
        {value === undefined && rows.length === 0 && (
          <p className="hint">
            Receives the previous step's whole output. Adding fields replaces it
            with an input object.
          </p>
        )}
        {value !== undefined && rows.length === 0 && (
          <p className="hint">
            An empty binding object supplies an empty input object.
          </p>
        )}
        {!advanced &&
          rows.map((row, index) => (
            <fieldset key={index} className={styles.field}>
              ...
              <div className={styles.fields}>
                <TextInput
                  ...
                  ref={(element) => element?.setCustomValidity(nameError(row))}
                  onChange={(event) =>
                ...
                <NativeSelect
                  ...
                  data={sources}
                  onChange={(event) => {
                    const source = event.target.value as InputBinding['source'];
                    patch(index, {
                      binding:
                        source === 'node'
        ...
        <Button
          variant="ghost"
          onClick={(event) => {
            const invalid = event.currentTarget
              .closest('details')
              ?.querySelector<HTMLInputElement>(':invalid');
            if (invalid) {
